  ...
  return (
    <Card className="bg-card border border-border shadow-sm md:shadow-lg max-h-[calc(100vh-8rem)] overflow-y-auto">
      ...
      <CardContent className="space-y-4">
        ...
        {/* Trade List - 1 line each (ONLY actual trades, not withdrawals) */}
        ...
        {/* Withdrawals Section - Show separately if any */}
            ...
            {withdrawals.map((withdrawal) => (
              ...
              >
                <div className="flex items-center gap-2 flex-1 min-w-0">
                  ...
                  {withdrawal.notes && (
                    <span className="text-[10px] text-muted-foreground truncate">{withdrawal.notes}</span>
                  )}
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <span className="text-sm text-muted-foreground">
                    -${Math.abs(withdrawal.pnl).toFixed(2)}
                  </span>
                  <Button
                    variant="ghost"
                    size="icon"
                    onClick={() => onDeleteTrade(withdrawal.id)}
                    className="size-7 text-muted-foreground hover:text-destructive"
                  >
                    <Trash2 className="size-4" />
                  </Button>
